...
public struct CodeScannerView: UIViewControllerRepresentable {
    ...
    public class ScannerCoordinator: NSObject, AVCaptureMetadataOutputObjectsDelegate {
        ...
        public func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
            if let metadataObject = metadataObjects.first {
                ...
                case .once:
                    found(result: ScanResult(rawBarcode: stringValue, error: nil, type: readableObject.type))
        ...
        func didFail(reason: ScanError) {
            let result = ScanResult(rawBarcode: nil, error: reason, type: nil)
            parent.completion(result)
        }
    }
    ...
    public class ScannerViewController: UIViewController,UIImagePickerControllerDelegate,UINavigationControllerDelegate{
        ...
        override public func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
            ...
            delegate?.found(result: ScanResult(rawBarcode: simulatedData))
        }
        ...
        public func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]){
            if let qrcodeImg = info[.originalImage] as? UIImage {
                ...
                if qrCodeLink=="" {
                    ...
                    delegate?.found(result: CodeScannerView.ScanResult(rawBarcode: qrCodeLink))
                }
            }
            else{
    ...
    public class ScannerViewController: UIViewController {
        ...
        private var videoCaptureDevice: AVCaptureDevice? = AVCaptureDevice.default(for: .video)
        var delegate: ScannerCoordinator?
        ...
        public func zoom(level: CGFloat){
            do{
                ...
                videoCaptureDevice?.videoZoomFactor = level
...
//            /*guard let */ self.videoCaptureDevice = AVCaptureDevice.default(for: .video) //else { return }
            //            videoCaptureDevice?.activeFormat = (videoCaptureDevice?.formats.last)!
    ...
    public var zoomLevel: Double?{
        ...
        set{
            viewController.zoom(level: CGFloat(newValue ?? 1))
            _zoomLevel = newValue
        }
    }
    ...
    public init(codeTypes: [AVMetadataObject.ObjectType], scanMode: ScanMode = .once, scanInterval: Double = 2.0, simulatedData: String = "", completion: @escaping (ScanResult) -> Void) {
